...
export default class Player {
    ...
    setCharacterClass(className) {
        console.log(`Setting character class to: ${className}`);
        
        // Clear any existing abilities first
        this.abilities = [];
        
        // Check if class is valid
        const validClasses = ["knight", "mage", "archer", "gambler"];
        if (!validClasses.includes(className)) {
            console.error(`Invalid class: ${className}, defaulting to knight`);
            className = "knight";
        }
        ...
        // Load sprites for this class
        try {
            this.loadClassSprites(className);
            console.log(`Loaded sprites for class: ${className}`);
        } catch (error) {
            console.error(`Error loading sprites for class ${className}:`, error);
        }
        
        // Apply class-specific stats
        this.applyClassStats(className);
        
        // Update base stats according to class
        switch(className) {
            case 'knight':
                this.baseAttack = 5;
                this.baseDefense = 3;
        ...
        // Set initial attack and defense values
        this.attack = this.baseAttack + (this.currentWeapon ? this.currentWeapon.attackValue : 0);
        this.defense = this.baseDefense + (this.currentArmor ? this.currentArmor.defenseValue : 0);

        // Initialize the class ability - THIS IS THE CRITICAL MISSING PIECE
        this.initClassAbility();
        console.log(`Initialized ${className} ability:`, this.abilities[0]?.name || "none");
