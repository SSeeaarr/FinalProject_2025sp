export default class CollisionChecker {
  ...
  checkEntity(entity, targets) {
    let index = 999;
    
    // Check if player is dashing (phasing) - only applies to the player
    const isPlayerDashing = entity === this.gp.player && 
                           entity.abilities.some(ability => 
                               ability.name === "Dash" && ability.phasing);
    
    // Skip monster collision check if player is phasing
    if (isPlayerDashing && targets === this.gp.monster) {
        return index;
    }
    
    const currentMapTarget = targets[entity.gp.currentMap];
