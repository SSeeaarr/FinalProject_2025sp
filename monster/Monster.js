export default class Monster {
    constructor(gp) {
        ...
        this.alive = true;      // Is the monster alive?
        this.dying = false;     // Is the monster in dying animation?
        this.deathCounter = 0;  // Counter for death animation
        this.deathAnimationDuration = 30; // Frames for death animation
        this.direction = "down";
        this.solidArea = {
            x: 3,
            y: 18,
    ...
    draw(ctx, gp) {
        // Only draw if the monster is alive or dying
        if (this.alive || this.dying) {
            // Get screen coordinates
            const screenX = this.x - gp.player.x + gp.player.screenX;
            const screenY = this.y - gp.player.y + gp.player.screenY;

            // Only draw if on screen
            if (this.isOnScreen(screenX, screenY, gp)) {
                let image = this.sprites[this.direction][this.spriteNum - 1];

                // If dying, you could add visual effects
                if (this.dying) {
                    // For example, make the monster fade out
                    ctx.globalAlpha = 1 - (this.deathCounter / this.deathAnimationDuration);

                    // Or use a death sprite if you have one
                    // image = this.deathSprite;
                }

                ctx.drawImage(image, screenX, screenY, gp.tileSize, gp.tileSize);
                ...
                // Draw health bar if monster is alive
                if (this.alive && !this.dying) {
                    this.drawHealthBar(ctx, screenX, screenY, gp);
                }
            }
        }
    }
    ...
    update() {
        // Only perform regular updates if the monster is alive and not dying
        if (this.alive && !this.dying) {
            if (this.dying) {
                this.dyingAnimation();
                return;
        ...
        // If monster is in dying state, handle the death animation
        else if (this.dying) {
            this.deathCounter++;

            // When death animation completes
            if (this.deathCounter >= this.deathAnimationDuration) {
                this.dying = false;
                this.alive = false;
                this.deathCounter = 0;
                // Set to null in the monster array later in game loop
            }
        }
    }
    ...
    giveRewards() {
        ...
        if (this.gp.player.exp >= this.gp.player.nextLevelExp) {
            ...
            setTimeout(() => {
                this.gp.ui.showMessage(`Dexterity increased to ${this.gp.player.dexterity}!`);
            }, 3000);
        }
