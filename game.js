...
class GamePanel {
    ...
    draw() {
        ...
        // Draw other players
        if (this.multiplayer) {
            console.log('Drawing other players, count:', this.multiplayer.otherPlayers.size);
    ...
    drawOtherPlayer(player) {
        ...
        const screenCenterX = this.screenWidth / 2 - this.tileSize / 2;
        const screenCenterY = this.screenHeight / 2 - this.tileSize / 2;
        
        // Use worldX/worldY for both players if available
        const myWorldX = this.player.worldX !== undefined ? this.player.worldX : this.player.x;
        const myWorldY = this.player.worldY !== undefined ? this.player.worldY : this.player.y;
        const playerWorldX = player.worldX !== undefined ? player.worldX : player.x;
        const playerWorldY = player.worldY !== undefined ? player.worldY : player.y;

        const offsetX = playerWorldX - myWorldX;
        const offsetY = playerWorldY - myWorldY;

        const drawX = screenCenterX + offsetX;
        const drawY = screenCenterY + offsetY;
        
        // Choose color based on class
        let playerColor;
        switch(player.characterClass) {
            case 'knight': playerColor = '#3366FF'; break; // Blue
            case 'mage': playerColor = '#CC33FF'; break;   // Purple
            case 'archer': playerColor = '#33CC33'; break; // Green
            case 'gambler': playerColor = '#FFCC00'; break; // Gold
            default: playerColor = '#FF6666'; break;       // Red
        }
        
        // Choose sprite based on direction
        let sprite;
        if (player.direction === 'up') {
            sprite = this.player.up1;
        } else if (player.direction === 'down') {
            sprite = this.player.down1;
        } else if (player.direction === 'left') {
            sprite = this.player.left1;
        } else {
            sprite = this.player.right1;
        }
        
        // Draw the sprite if available, otherwise use rectangle
        ...
        const displayName = player.name || player.id.split('-')[0];
    ...
    enableDebugMode() {
        ...
        setInterval(() => {
            ...
            let debugText = `Player: (${this.player.x.toFixed(0)},${this.player.y.toFixed(0)})<br>`;
            ...
            if (this.multiplayer) {
                ...
                this.multiplayer.otherPlayers.forEach((player, id) => {
                    debugText += `- ${player.name}: (${player.x.toFixed(0)},${player.y.toFixed(0)})<br>`;
                });
            }
            
            document.getElementById('debug-overlay').innerHTML = debugText;
        }, 1000);
    }
}
...
class MultiplayerManager {
    ...
    handleMessage(message) {
        // Add this at the beginning of handleMessage
    ...
    updatePlayerPosition(data) {
        console.log('Received position update for player:', data.senderId, data);
        
        const { senderId, x, y, direction, characterClass, name } = data;
        
        if (!this.otherPlayers.has(senderId)) {
            console.log('Adding new player to otherPlayers map:', senderId);
        ...
        // Also update x and y for backward compatibility
        player.x = x;
        player.y = y;
    ...
    addPlayer(data) {
        const { senderId, characterClass, name } = data;
